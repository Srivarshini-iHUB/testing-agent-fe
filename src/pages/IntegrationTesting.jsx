      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {step === "report" && report && (
          <div className="animate-in fade-in duration-500">
            ...
            {/* Fixture Details */}
            {report.fixture_details && report.fixture_details.length > 0 && (
              <div className="p-6 bg-slate-800/50 border border-slate-700/50 rounded-xl hover:border-slate-600/50 transition-all duration-300 mb-10">
                <h3 className="text-lg font-bold text-white mb-6">Fixture Details</h3>
                <div className="space-y-4">
                  {report.fixture_details.map((fixture, idx) => (
                    <div
                      key={idx}
                      className="p-5 rounded-lg border bg-slate-700/30 border-slate-600/50"
                    >
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex items-center gap-3 flex-1">
                          <span className="text-slate-400 flex-shrink-0 font-mono text-sm">{fixture.fixture}</span>
                        </div>
                        <span className={`px-3 py-1.5 rounded-lg text-xs font-bold flex-shrink-0 ${
                          fixture.status_code >= 200 && fixture.status_code < 300 ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"
                        }`}>
                          {fixture.status_code}
                        </span>
                      </div>
